...
::: {.info title="Propósito da edição V4"}
Esta edição transforma o método em um caminho executável no Cursor: documento caótico, artefatos canônicos, montagem de contexto, execução observada, Eval Spec e Evidence Record validado. A metáfora da oficina continua construindo o modelo mental; o backend mostra o uso real; os laboratórios finais entregam o corpus e o harness de avaliação completos.
:::
...
![Do manual caótico ao sistema consultável.](assets/diagrams/01-caos-separacao.png){ width=95% }
...
![Do pedido ao Evidence Record no runtime de referência.](assets/diagrams/02-runtime-cursor.png){ width=95% }
...
O objetivo SMART do checkpoint é: **até o final desta seção, executar uma consulta no Cursor, registrar quais dos quatro arquivos foram consultados e reprovar o experimento se a resposta afirmar conclusão assíncrona sem estado durável consultável**.
...
![Produção e consumo dos campos de um Eval Spec.](assets/diagrams/04-eval-evidence.png){ width=95% }
...
- **Fase 1 - POC:** escolher um domínio, um agent card curto, dois playbooks frequentes, uma skill determinística, uma rule crítica, um contract, uma suíte pequena de evals e um formato de Evidence Record.
- **Fase 2 - piloto:** medir sucesso e retrabalho, comparar custo por tarefa concluída, inspecionar falsos positivos, entrevistar operadores e corrigir nomenclatura ou navegação.
- **Fase 3 - padrão controlado:** definir curadoria, automatizar validações estruturais, versionar contracts e evals, publicar catálogo e acompanhar drift ou conteúdo órfão.

![Ciclo de adoção orientada por evidência.](assets/diagrams/06-adocao-loop.png){ width=95% }
